...
public final class InvertedRectangleBatch implements QuickImports, AutoCloseable {
   public static final int MAX_BATCH = 32;
   private static final ShaderProgramKey SHADER_KEY = new ShaderProgramKey(
      Identifier.of("minecraft", "core/round_inverted_batch"), VertexFormats.POSITION, Defines.EMPTY
   );
   ...
   private static InvertedRectangleBatch ACTIVE = null;
   private static final InvertedRectangleBatch INSTANCE = new InvertedRectangleBatch();
   private final float[] locations = new float[64];
   private final float[] sizes = new float[64];
   private final float[] radii = new float[128];
   private final float[] quadCoords = new float[256];
   private int count = 0;
   private boolean closed = false;

   public static InvertedRectangleBatch active() {
      return ACTIVE;
   }

   public static InvertedRectangleBatch begin() {
      if (ACTIVE != null) {
         ACTIVE.end();
      }
   ...
   private InvertedRectangleBatch() {
   }
   ...
   public void submit(ShapeProperties shape) {
      if (!this.closed) {
         ...
         this.radii[i4 + 0] = round.x;
         this.radii[i4 + 1] = round.y;
         this.radii[i4 + 2] = round.z;
         this.radii[i4 + 3] = round.w;
         this.count++;
      }
   }
   ...
   public void flush() {
      if (this.count != 0) {
         for (int i = this.count; i < 32; i++) {
            ...
            this.radii[i * 4] = 0.0F;
            this.radii[i * 4 + 1] = 0.0F;
            this.radii[i * 4 + 2] = 0.0F;
            this.radii[i * 4 + 3] = 0.0F;
         }
