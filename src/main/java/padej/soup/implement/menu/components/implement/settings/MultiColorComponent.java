...
public class MultiColorComponent extends AbstractSettingComponent {
   ...
   public void render(DrawContext context, int mouseX, int mouseY, float delta) {
      ...
      if (this.setting.isVisible()) {
         ...
         int colorCount = this.setting.getColorCount();
         String wrapped = StringUtil.wrap(this.setting.getLocalizedName(), (int)(this.width - colorCount * 9 - 18.0F - textOffset), 14);
         ...
         for (int i = 0; i < colorCount; i++) {
            ColorSetting colorSetting = this.setting.getColor(i);
            if (colorSetting != null) {
               float colorX = this.x + this.width - 15.5F - (colorCount - 1 - i) * 9;
               float colorY = this.y + this.height / 2.0F - 3.5F;
               rectangle.render(ShapeProperties.create(matrix, colorX, colorY, 7.0, 7.0).round(3.5F).color(colorSetting.getColor()).build());
               rectangle.render(
                  ShapeProperties.create(matrix, colorX, colorY, 7.0, 7.0)
                     .round(3.5F)
                     .thickness(2.0F)
                     .softness(1.0F)
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      if (!this.setting.isVisible()) {
         ...
         for (int i = 0; i < colorCount; i++) {
            float colorX = this.x + this.width - 15.5F - (colorCount - 1 - i) * 9;
            float colorY = this.y + this.height / 2.0F - 3.5F;
            if (MathUtil.isHovered(mouseX, mouseY, colorX, colorY, 7.0, 7.0)) {
               if (button == 0) {
                  this.setting.setSelectedColorIndex(i);
                  ColorSetting selectedColor = this.setting.getColor(i);
               ...
               if (button == 1) {
                  this.setting.setSelectedColorIndex(i);
                  SoundManager.playSound(SoundManager.CLICK, 1.0F, 1.0F);
                  return true;
               }
            }
         }
   ...
   public boolean isHover(double mouseX, double mouseY) {
      if (!this.setting.isVisible()) {
         ...
         return MathUtil.isHovered(mouseX, mouseY, this.x + 9.0F, this.y + 6.0F, this.width - colorCount * 9 - 18.0F, this.height - 12.0F);
      }
   }
}
