...
public final class HitBubblesRenderer implements QuickImports {
   private static final MatrixStack SCRATCH_MATRIX = new MatrixStack();
   private static final Vector4i SCRATCH_COLORS = new Vector4i();

   public static void renderBubble(Vec3d pos, float yaw, float pitch, StopWatch timer, float rotationAngle, HitBubbles module, ThreeStageAnimation animation) {
      if (animation != null) {
         ...
         if (!animation.isFinished(elapsedSeconds)) {
            ...
            float alpha = calculateAlpha(animationValue, module);
            float scale = calculateScale(animationValue, module);
            Vector4i colors = getBubbleColors(alpha, module);
